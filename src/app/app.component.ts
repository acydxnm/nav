...
export class AppComponent implements OnInit, AfterViewInit {
    ...
    if (isSelfDevelop) {
      getContentes().then(() => {
        setTimeout(() => {
          ...
          const hasDefault = routes.find((item: any) => item.path === defaultTheme);
  ...
  private goRoute() {
    if (settings.appTheme !== 'Current' && isMobile()) {
      if (location.href.includes('system')) return;
  ...
  private registerKeyboard() {
    document.addEventListener('keyup', (e) => {
      ...
      if (key === createWebKey) {
        event.emit('CREATE_WEB', { isKeyboard: true });
      }
    });
  }
}
